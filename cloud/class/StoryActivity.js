...
const ParseObject = Parse.Object.extend('StoryActivity');
...
function afterSave(req, res) {
    ...
    if (req.object.get('comment')) {
        promises.push(new Parse.Query('StoryComment').equalTo('objectId', req.object.get('comment').id).first({
            useMasterKey: true
        }));
    }
...
function create(obj, acl) {
    ...
    if (obj.story) {
        newActivity.set('story', obj.story);
    }
